...
public class StructuredConcurrency {
  ...
  public static void findWithStructuredConcurrencyShutdownOnFailure() throws InterruptedException, ExecutionException {
    ...
    try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
      StructuredTaskScope.Subtask<String> findUserSubtask = scope.fork(() -> findUser());
      StructuredTaskScope.Subtask<Integer> fetchOrderSubtask = scope.fork(() -> fetchOrder());
  ...
  private static NetworkConnection tryToGetWifi() {
    sleepSeconds(5);
    return new NetworkConnection("WiFi");
  }

  private static NetworkConnection tryToGet5g() {
    sleepSeconds(3);
    return new NetworkConnection("5G");
  }

  private static NetworkConnection tryToGet4g() {
    sleepSeconds(4);
